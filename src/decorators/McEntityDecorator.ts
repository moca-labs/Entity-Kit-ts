...
} from "../core/McEntityCore";
import { isFunction, isObject, isString, isSymbol } from "../core/McTypeUtils";
...
const applyLegacyEntityWrapper = (path: string | undefined, ctor: Constructor): Constructor => {
	...
	for (const key of Reflect.ownKeys(legacyMeta)) {
		if (key === FIELD_KEYS) {
			...
			const parentIgnores = (parentMeta?.[SERIALIZE_IGNORE_FLAG] as string[] | undefined) ?? [];
 ...
 */
export function ENTITY(pathOrValue?: string | (new (...args: any[]) => any), context?: ClassDecoratorContext): any {
	...
	return (value: new (...args: any[]) => any, ctx?: ClassDecoratorContext): any => {
