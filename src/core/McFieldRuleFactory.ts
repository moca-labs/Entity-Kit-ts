...
export class McFieldRuleFactory {
	static typed(type: any, isArray: boolean, path: string | undefined, defaultValue: any): McFieldRule {
		return isArray ? new McArrayFieldRule(type, path, defaultValue) : new McScalarFieldRule(type, path, defaultValue);
	}

	static map(type: any, isArray: boolean, mapKeyType: any, path: string | undefined): McFieldRule {
		return new McMapFieldRule(type, isArray, mapKeyType, path, undefined);
	}
