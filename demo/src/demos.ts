...
const toPlain = (val: unknown, depth = 0): unknown => {
  ...
  if (val instanceof Map)
    return Object.fromEntries([...val.entries()].map(([k, v]) => [String(k), toPlain(v, depth + 1)]));
...
class Dashboard {
  ...
  @McEntity.MAP_FIELD(Permission, "rolePermissions", Number) rolePermissions: Map<number, Permission> = new Map();
...
class Order {
  ...
  @McEntity.CUSTOM_FIELD((_: any, raw: number) => raw / 10_000, "amount_cents") amountManwon = 0;
  @McEntity.CUSTOM_FIELD((_: any, raw: string) => new Date(raw), "created_at") createdAt: Date = new Date(0);
  @McEntity.CUSTOM_FIELD((self: any, raw: string) => `[${raw}] ${self.id}`) label = "";
}
...
const profileInput = {
  name: "Alice",
  wallet: { user_id: "u-001", balance: 1_250_000, isActive: true, internal_token: "tok_secret_xyz" },
};
...
const d07searchResponse = {
  code: 0,
  data: { result: { total: 128, items: ["TypeScript", "JavaScript", "Decorator"] } },
};
    ...
    scenarios: [
      {
        ...
        input: {
          name: "Bob",
          salary: 5_000_000,
          company: { name: "Moca Labs", address: { city: "Seoul", street: "Gangnam-daero 123" } },
        },
        output: toPlain(
          mk(Employee, {
            name: "Bob",
...
parseTags(raw: string): string[] {
    ...
    scenarios: [
      {
        ...
        output: toPlain(
          mk(Order, { id: "ORD-001", amount_cents: 250_000, created_at: "2026-06-18T09:00:00Z", label: "PREMIUM" })
        ),
      },
      {
        ...
        output: toPlain(
          mk(Post, { title: "Native Decorators 완벽 분석", raw_tags: "TypeScript, decorator, Symbol.metadata" })
        ),
      },
    ],
  },

  {
    ...
    codeSnippet: `@McEntity.ENTITY
class Wallet extends McSerializable {
  @McEntity.FIELD(String, "user_id") @McEntity.SERIALIZE
  userId = "";

  @McEntity.FIELD(Number) @McEntity.SERIALIZE("balance_krw")
  balance = 0;

  @McEntity.FIELD(Boolean) @McEntity.SERIALIZE
  isActive = false;
